...
export default function Input({ name, placeholder, visibility, handlerClear }: { name: string; placeholder: string; visibility: boolean; handlerClear: () => void }) {
  return (
    <label className="pointer-events-auto relative">
      ...
      {name === 'date-start' && !visibility ? (
        ...
        <button className={`absolute right-3 top-1/2 -translate-y-1/2 text-[#0c73fe] hover:text-[#ff6f32] ${visibility ? 'block' : 'hidden'}`} type="button" onClick={handlerClear}>
